...
def index(request: Request, wallboard: int = 0):
    return templates.TemplateResponse("index.html", {"request": request, "wallboard": bool(wallboard)})


@app.get("/wallboard", response_class=HTMLResponse)
def wallboard(request: Request):
    return templates.TemplateResponse("index.html", {"request": request, "wallboard": True})
...
def _render_daily_report_html(report: dict[str, Any]) -> str:
    ...
    return f"""
    <!doctype html>
    <html><head><meta charset="utf-8"/><title>Daily SOC Report</title>
    <style>
      ...
      <h1>Daily SOC Report</h1>
      <div class="meta">Generated at {report['generated_at']}</div>
      <div class="kpi"><strong>Total Logs:</strong> {s['total_logs']}</div>
      <div class="kpi"><strong>Total Alerts:</strong> {s['total_alerts']}</div>
      <div class="kpi"><strong>High Alerts:</strong> {s['high_alerts']}</div>
      <div class="kpi"><strong>Open Alerts:</strong> {s['open_alerts']}</div>
      <div class="kpi"><strong>Critical Assets Open:</strong> {s.get('critical_open_assets', 0)}</div>
      <h2>Latest Alerts</h2>
      <table>
        <thead><tr><th>Time</th><th>Severity</th><th>Type</th><th>IP</th><th>Status</th><th>Occ</th><th>Details</th></tr></thead>
...
@app.post("/api/cases/{case_id}/alerts/{alert_id}")
...
@app.delete("/api/cases/{case_id}/alerts/{alert_id}")
...
@app.delete("/api/cases/{case_id}/comments/{comment_id}")
...
def alert_context(alert_id: int):
    ...
    related_logs = fetch_all(
        """
        SELECT id, ts, ip, method, path, status_code, user_agent, message
        FROM logs
        WHERE (? IS NULL OR ip = ?)
          AND ts BETWEEN ? AND ?
        ORDER BY ts DESC
...
def daily_report_page():
    report = daily_report_data()
    return HTMLResponse(_render_daily_report_html(report))
...
@app.post("/api/live-tail/stop")
def stop_live_tail():
    state = tail_manager.stop()
    return state.__dict__


@app.get("/api/live-tail/status")
